...
class _QuizPageState extends State<QuizPage> {
  final String apiUrl =
      "https://translation.saeedantechpvt.com/api/app/get-questions?word=help&type=english to soomaali";
  ...
  Future<void> fetchQuizData(List<String> word, String selectedLanguage) async {
    try {
      final response = await http.post(
        Uri.parse(
            "https://translation.saeedantechpvt.com/api/app/get-questions"),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
            ...
            content: Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    ' ${(correctAnswers / quizData.length * 100).toStringAsFixed(2)}% Score',
                    style:
                        const TextStyle(color: Color(0xFF4209BB), fontSize: 48),
                  ),

                  widget.language == "soomaali"
                      ? const Text(
                          'Hambalyo! ',
                          style: TextStyle(
                  ...
                  widget.language == "english"
                      ...
                      : widget.language == "soomaali"
                          ? const Text(
                              'Imtixaanki guul baad ku dhameysay',
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 20,
                                  color: Colors.black),
                  ...
                  widget.language == "english"
                      ? Text(
                          'You Attempted  ${quizData.length} questions and  $correctAnswers are answered correct'
                              .tr,
                      ...
                      : widget.language == "soomaali"
                          ? Text(
                              'Waxaad isku dayday  ${quizData.length} su aalood  $correctAnswers na si sax ah ayaad uga jawaabtay',
                              style: const TextStyle(
                                color: Colors.black,
                                fontSize: 25,
                              ),
                          ...
                          : widget.language == "arabic"
                              ? Text(
                                  'لقد حاولت   ${quizData.length}  وتمت اإلجابة على  $correctAnswers منها بشكل صحيح',
                                  style: const TextStyle(
                                    color: Colors.black,
                                    fontSize: 25,
                                  ),
                    ...
                    Row(
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            await fetchQuizData(
                                userController.words.value, selectedLanguage);
                          },
                          style: ElevatedButton.styleFrom(
                        ...
                        ElevatedButton(
                          onPressed: () async {
                            ...
                            await fetchQuizData(userController.nullList.value,
                                selectedLanguage);
                          },
                          style: ElevatedButton.styleFrom(
